...
export const ConfigurationTable = ({ data, onDelete }: ConfigurationTableProps): React.ReactElement => {
  ...
  const handleRowClick = (id: number) => {
    router.push(`/admin/application-configuration/country-configuration/${id}`);
  };
    ...
    <>
      <TableWrapper TableHeadData={['S no', 'Title',  'Action']}>
        {data && data.map((td, index) => {
          return (
            <tr key={td.id} className="border-2 border-boxOutline cursor-pointer" onClick={() => handleRowClick(td.id)}>
